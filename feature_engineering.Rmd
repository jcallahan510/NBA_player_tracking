...
shot_attempts_all %>% 
  mutate(nearest_def_dist = sqrt(((p2_x_loc-p3_x_loc)^2) + ((p2_y_loc-p3_y_loc)^2)),
         shot_dist_from_L_hoop = sqrt(((p2_x_loc-5.35)^2) + ((p2_y_loc-25)^2)),
         shot_dist_from_R_hoop = sqrt(((p2_x_loc-88.65)^2) + ((p2_y_loc-25)^2)),
         def_dist_from_L_hoop = sqrt(((p3_x_loc-5.35)^2) + ((p3_y_loc-25)^2)),
         def_dist_from_R_hoop = sqrt(((p3_x_loc-88.65)^2) + ((p3_y_loc-25)^2)),
         shooter_hoop = ifelse(shot_dist_from_L_hoop < shot_dist_from_R_hoop, shot_dist_from_L_hoop, shot_dist_from_R_hoop),
         def_hoop = ifelse(def_dist_from_L_hoop < def_dist_from_R_hoop, def_dist_from_L_hoop, def_dist_from_R_hoop),
         def_angle = acos_d(((nearest_def_dist^2)+(shooter_hoop^2)-(def_hoop^2))/(2*nearest_def_dist*shooter_hoop))
         
         
         ) %>% 
  select(def_angle, shooter_hoop, def_hoop, nearest_def_dist, shot_dist_from_L_hoop, shot_dist_from_R_hoop, def_dist_from_L_hoop, def_dist_from_R_hoop, everything())



def_angle(79.28948, 33.02286, 79.42200, 26.53512)
