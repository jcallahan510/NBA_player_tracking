...
curr_dir <- '/data/p_dsi/nba_player_movement/data_unzipped/data_unzipped/csv_folder_2'

files_batch_2 <- NULL

for(i in curr_dir){
  files <- list.files(path = i, pattern = "*.csv", full.names=TRUE, recursive=FALSE)
  files_batch_2 <- append(files_batch_2, files)
}

#files_batch_2 <- files_batch_2[-20]
...
```{r}
shot_attempts_batch_2 <- NULL
count <- 0


for(i in files_batch_2){
  ...
  shot_attempts_batch_2 <- bind_rows(shot_attempts_final, shot_attempts_batch_2)
...
```{r}
shot_attempts_batch_2 %>% 
  group_by(game_id) %>% 
  summarise(n = n())

write.csv(shot_attempts_batch_2, 'shot_attempts_batch_2.csv', row.names = FALSE)
```
