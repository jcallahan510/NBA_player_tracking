...
for(i in curr_dir){
  ...
  files_batch_4 <- append(files_batch_4, files)
}

#files_batch_4 <- files_batch_4[-20]
...
```{r}
shot_attempts_batch_4 <- NULL
count <- 0


for(i in files_batch_4){
  ...
  shot_attempts_batch_4 <- bind_rows(shot_attempts_final, shot_attempts_batch_4)

  count <- count+1
  print(count)
}

shot_attempts_batch_4 <- shot_attempts_batch_4 %>% 
  distinct()
...
```{r}
shot_attempts_batch_4 %>% 
  group_by(game_id) %>% 
  summarise(n = n())

write.csv(shot_attempts_batch_4, 'shot_attempts_batch_4.csv', row.names = FALSE)
```
